...
const { help, snk, info, donate, deks, listChannel } = require('./lib/help')
...
module.exports = msgHandler = async (client, message) => {
    try {
        ...
        const ownerNumber = '6285860280863@c.us'
        ...
        case '$donasi':
        case '$donate':
            client.sendLinkWithAutoPreview(from, 'GOPAY : 081572880106 :)', donate)
            break
        case '$tts':
        ...
        case '$fb':
            ...
            break
        case '$creator':
            client.sendContact(from, '6281572880106@c.us')
            break
        case '$ig':
        ...
        case '$nsfwmenu':
            ...
            break
        case '$igstalk':
            if (args.length === 1)  return client.reply(from, 'Kirim perintah *$igStalk @username*\nConntoh *$igStalk @kingoeproject*', id)
        ...
        case '$bc':
            ...
            for (let ids of chatz) {
                ...
                if (!cvk.isReadOnly) await client.sendText(ids, `[ Kingoe Project BoT ]\n\n${msg}`)
            }
        ...
        case '$leave':
            if (!isGroupMsg) return client.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return client.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
            await client.sendText(from,'Assalamu'allaikum).then(() => client.leaveGroup(groupId))
            break
        case '$promote':
            if (!isGroupMsg) return client.reply(from, 'Fitur ini hanya bisa di gunakan dalam group', id)
